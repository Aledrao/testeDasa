...
public class ExameServiceImplTest {
	...
	@BeforeEach
	public void setup() {
		mockMvc = MockMvcBuilders.standaloneSetup(exameService)
	...
	@Test
	public void deveriaBuscarListaExame_sucesso() throws Exception {
		Laboratorio laboratorio = new Laboratorio();
		laboratorio.setCodigoLaboratorio(1L);
		laboratorio.setNomeLaboratorio("Dasa");
		
		Exame exame = new Exame();
		exame.setCodigoExame(1L);
		exame.setNomeExame("Contagem Plaquetas");
		exame.setLaboratorio(laboratorio);
		
		List<Exame> exames = new ArrayList<>();
		exames.add(exame);
		
		Mockito.when(exameRepository.findByCodigoExame(1L)).thenReturn(exames);
		
		List<Exame> response = exameService.listaExames(1L);
